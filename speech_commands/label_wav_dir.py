...
def run_graph(wav_dir, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    for wav_path in glob.glob(wav_dir + '/*.wav'):
      ...
      pred = {'filename' : wav_path}
      for node_id in top_k:
        human_string = labels[node_id]
        score = predictions[node_id]
        #print('%s (score = %.5f)' % (human_string, score))
        pred[human_string] = score
      results.append(pred)
    sess.close()
...
def label_wav(wav_dir, labels, graph, input_name, output_name, how_many_labels):
  ...
  results = run_graph(wav_dir, labels_list, input_name, output_name, how_many_labels)
  return results


def main(FLAGS):
  """Entry point for script, converts flags to arguments."""
  return label_wav(FLAGS['wav_dir'], FLAGS['labels'], FLAGS['graph'], FLAGS['input_name'],
